  ...
  Object.keys(character.base_stats).forEach((stat) => {
    if (flats[stat]) {
      character.stats.flat.skill_milestones[stat] =
        (character.stats.flat.skill_milestones[stat] || 0) + flats[stat];
    }
    if (multipliers[stat]) {
...
character.stats.add_all_equipment_bonus = function () {
  ...
  if (character.equipment.weapon == null) {
    character.stats.multiplier.skills.attack_power =
      skills["Unarmed"].get_coefficient();
    character.stats.multiplier.skills.attack_speed =
      skills["Unarmed"].get_coefficient("multiplicative") ** 0.3333;
    ...
    character.stats.multiplier.skills.attack_power =
      skills[
        weapon_type_to_skill[character.equipment.weapon.weapon_type]
      ].get_coefficient();
    character.stats.multiplier.skills.attack_points =
      skills[
        weapon_type_to_skill[character.equipment.weapon.weapon_type]
      ].get_coefficient() ** 0.3333;
  }
};
...
character.stats.add_all_skill_level_bonus = function () {
  ...
  character.stats.multiplier.skills.stamina_efficiency =
    skills["Running"].get_coefficient("multiplicative");
  character.stats.multiplier.skills.strength =
    skills["Weightlifting"].get_coefficient("multiplicative");
  character.stats.multiplier.skills.block_strength =
    1 + 5 * skills["Shield blocking"].get_level_bonus();
  character.stats.multiplier.skills.agility =
    skills["Equilibrium"].get_coefficient("multiplicative");
};
...
character.stats.add_location_penalties = function () {
  ...
  if (current_location) {
    ...
    if (
      ...
      (current_location.light_level === "normal" &&
        (current_game_time.hour >= 20 || current_game_time.hour <= 3))
    ) {
      light_modifier =
        0.5 +
        (0.5 * skills["Night vision"].current_level) /
  ...
  if (damage_value < 1) {
    ...
    damage_taken =
      Math.ceil(
        10 *
          Math.max(
            damage_value - character.stats.full.defense,
            damage_value * 0.1,
            1
          )
      ) / 10;
...
function add_to_character_inventory(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    if (
      items[i].item.tags.tool &&
      character.equipment[items[i].item.equip_slot] === null
    ) {
