      ...
      if (
        ...
        while (this.total_xp >= this.total_xp_to_next_lvl) {
          ...
          if (this.rewards?.milestones[level_after_xp]?.unlocks?.skills) {
            unlocks.skills.push(
              ...this.rewards.milestones[level_after_xp].unlocks.skills
            );
          }
        ...
        if (
          Object.keys(gains.flats).length > 0 ||
          Object.keys(gains.multipliers).length > 0 ||
          Object.keys(gains.xp_multipliers).length > 0
        ) {
          message += `<br><br> Thanks to ${this.name()} reaching new milestone, ${
            character.name
          } gained: `;

          if (gains.flats) {
            Object.keys(gains.flats).forEach((stat) => {
              message += `<br> +${gains.flats[stat]} ${stat_names[stat].replace(
                "_",
                " "
              )}`;
            });
          ...
          if (gains.multipliers) {
            Object.keys(gains.multipliers).forEach((multiplier) => {
              message += `<br> x${
                Math.round(100 * gains.multipliers[multiplier]) / 100
              } ${stat_names[multiplier].replace("_", " ")}`;
            });
          }
      ...
      if (this.rewards?.milestones[i]) {
        ...
        if (multipliers) {
          Object.keys(multipliers).forEach((multiplier) => {
            gains.multipliers[multiplier] =
              (gains.multipliers[multiplier] || 1) * multipliers[multiplier];
          });
        }
        if (xp_multipliers) {
          Object.keys(xp_multipliers).forEach((multiplier) => {
            gains.xp_multipliers[multiplier] =
              (gains.xp_multipliers[multiplier] || 1) *
    ...
    switch (scaling_type) {
      case "flat":
        return (
          1 +
          Math.round(
            (((this.max_level_coefficient - 1) * this.current_level) /
              this.max_level) *
              1000
          ) /
            1000
...
function get_unlocked_skill_rewards(skill_id) {
  ...
  if (skills[skill_id].rewards) {
    //rewards
    const milestones = Object.keys(skills[skill_id].rewards.milestones).filter(
      (level) => level <= skills[skill_id].current_level
    );
    if (milestones.length > 0) {
      unlocked_rewards = `lvl ${milestones[0]}: ${format_skill_rewards(
        skills[skill_id].rewards.milestones[milestones[0]]
      )}`;
      for (let i = 1; i < milestones.length; i++) {
        unlocked_rewards += `<br>\n\nlvl ${
          milestones[i]
        }: ${format_skill_rewards(
          skills[skill_id].rewards.milestones[milestones[i]]
        )}`;
      }
    }
  } else {
...
function format_skill_rewards(milestone) {
  ...
  if (milestone.multipliers) {
    ...
    if (formatted) {
      formatted += `, x${milestone.multipliers[multipliers[0]]} ${
    ...
    for (let i = 1; i < multipliers.length; i++) {
      formatted += `, x${milestone.multipliers[multipliers[i]]} ${
...
(function () {
  skills["Combat"] = new Skill({
    ...
    base_xp_cost: 30,
    get_effect_description: () => {
      return `Multiplies hit chance by ${
        Math.round(skills["Combat"].get_coefficient("multiplicative") * 1000) /
        1000
      }`;
    },
  });
  ...
  skills["Pest killer"] = new Skill({
    ...
    base_xp_cost: 50,
    get_effect_description: () => {
      return `Multiplies hit chance against small-type enemies by ${
        Math.round(
          skills["Pest killer"].get_coefficient("multiplicative") * 1000
        ) / 1000
      }`;
    },
    rewards: {
  ...
  skills["Giant slayer"] = new Skill({
    ...
    max_level_coefficient: 2,
    get_effect_description: () => {
      return `Multiplies evasion against large-type enemies by ${
        Math.round(
          skills["Giant slayer"].get_coefficient("multiplicative") * 1000
        ) / 1000
      }`;
    },
  });
  ...
  skills["Evasion"] = new Skill({
    ...
    base_xp_cost: 10,
    get_effect_description: () => {
      return `Multiplies your evasion chance by ${
        Math.round(skills["Evasion"].get_coefficient("multiplicative") * 1000) /
        1000
      }`;
    },
    rewards: {
  ...
  skills["Shield blocking"] = new Skill({
    ...
    get_effect_description: () => {
      return `Increases block chance by flat ${
        Math.round(skills["Shield blocking"].get_level_bonus() * 1000) / 10
      }%. Increases blocked damage by ${
        Math.round(skills["Shield blocking"].get_level_bonus() * 5000) / 10
      }%`;
    },
  });
  ...
  skills["Unarmed"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt in unarmed combat by ${
        Math.round(skills["Unarmed"].get_coefficient("multiplicative") * 1000) /
        1000
      }. 
Multiplies attack speed and AP in unarmed combat by ${
        Math.round(
          skills["Unarmed"].get_coefficient("multiplicative") ** 0.3333 * 1000
        ) / 1000
      }`;
    },
  ...
  skills["Spatial awareness"] = new Skill({
    ...
    get_effect_description: () => {
      return `Reduces environmental penalty in open areas by ^${
        Math.round(
          100 -
            (100 * skills["Spatial awareness"].current_level) /
  ...
  skills["Tight maneuvers"] = new Skill({
    ...
    get_effect_description: () => {
      return `Reduces environmental penalty in narrow areas by ^${
        Math.round(
          100 -
            (100 * skills["Tight maneuvers"].current_level) /
  ...
  skills["Night vision"] = new Skill({
    ...
    get_effect_description: () => {
      return `Reduces darkness penalty (except for 'pure darkness') by ^${
        Math.round(
          100 -
            (100 * skills["Night vision"].current_level) /
  ...
  skills["Presence sensing"] = new Skill({
    ...
    get_effect_description: () => {
      return `Reduces extreme darkness penalty by ^${
        Math.round(
          100 -
            (100 * skills["Presence sensing"].current_level) /
  ...
  skills["Swords"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with swords by ${
        Math.round(skills["Swords"].get_coefficient("multiplicative") * 1000) /
        1000
      }.
Multiplies AP with swords by ${
        Math.round(
          skills["Swords"].get_coefficient("multiplicative") ** 0.3333 * 1000
        ) / 1000
      }`;
    },
    rewards: {
  ...
  skills["Axes"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with axes by ${
        Math.round(skills["Axes"].get_coefficient("multiplicative") * 1000) /
        1000
      }.
Multiplies AP with axes by ${
        Math.round(
          skills["Axes"].get_coefficient("multiplicative") ** 0.3333 * 1000
        ) / 1000
      }`;
    },
    rewards: {
  ...
  skills["Spears"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with spears by ${
        Math.round(skills["Spears"].get_coefficient("multiplicative") * 1000) /
        1000
      }.
Multiplies AP with spears by ${
        Math.round(
          skills["Spears"].get_coefficient("multiplicative") ** 0.3333 * 1000
        ) / 1000
      }`;
    },
    rewards: {
  ...
  skills["Hammers"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with battle hammers by ${
        Math.round(skills["Hammers"].get_coefficient("multiplicative") * 1000) /
        1000
      }.
Multiplies AP with hammers by ${
        Math.round(
          skills["Hammers"].get_coefficient("multiplicative") ** 0.3333 * 1000
        ) / 1000
      }`;
    },
    rewards: {
  ...
  skills["Daggers"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with daggers by ${
        Math.round(skills["Daggers"].get_coefficient("multiplicative") * 1000) /
        1000
      }.
Multiplies AP with daggers by ${
        Math.round(
          skills["Daggers"].get_coefficient("multiplicative") ** 0.3333 * 1000
        ) / 1000
      }`;
    },
    rewards: {
  ...
  skills["Wands"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealt with wands by ${
        Math.round(skills["Wands"].get_coefficient("multiplicative") * 1000) /
        1000
      }`;
    },
    max_level_coefficient: 8,
  ...
  skills["Staffs"] = new Skill({
    ...
    get_effect_description: () => {
      return `Multiplies damage dealth with staffs by ${
        Math.round(skills["Staffs"].get_coefficient("multiplicative") * 1000) /
        1000
      }`;
    },
    max_level_coefficient: 8,
  ...
  skills["Iron skin"] = new Skill({
    ...
    get_effect_description: () => {
      return `Increases base defense by ${Math.round(
        skills["Iron skin"].get_level_bonus()
      )}`;
    },
    rewards: {
      milestones: {
  ...
  skills["Persistence"] = new Skill({
    ...
    get_effect_description: () => {
      return `Increases low stamina stat multiplier to x${
        (50 +
          Math.round(skills["Persistence"].get_level_bonus() * 100000) / 1000) /
        100
      } (originally x0.5)`;
    },
    rewards: {
  ...
  skills["Haggling"] = new Skill({
    ...
    get_effect_description: () => {
      return `Lowers trader cost multiplier to ${Math.round(
        (1 - skills["Haggling"].get_level_bonus()) * 100
      )}% of original value`;
    },
    max_level_bonus: 0.5,
  });
